...
function LangSwitcher({ lang, setLang }: { lang: 'de' | 'fr'; setLang: (l: 'de' | 'fr') => void }) {
  return (
    <div className="flex items-center gap-1 bg-black/60 backdrop-blur-2xl px-2.5 py-1 rounded-full border border-white/10 shadow-lg">
      <Languages className="w-3.5 h-3.5 text-zinc-400" />
      <button
        onClick={() => setLang('de')}
        className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase transition ${
          lang === 'de' ? 'bg-white text-black shadow' : 'text-zinc-400 hover:text-zinc-200'
        }`}
      >
        DE
      </button>
      ...
      <button
        onClick={() => setLang('fr')}
        className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase transition ${
          lang === 'fr' ? 'bg-white text-black shadow' : 'text-zinc-400 hover:text-zinc-200'
        }`}
      >
        FR
      </button>
    ...
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            >
              ...
              <div className="pt-6 border-t border-white/10 flex items-center justify-between">
                ...
                <a 
                  href={`tel:${displayPhone}`}
                  className="inline-flex items-center gap-2 px-5 py-2.5 text-black font-bold rounded-xl text-xs transition shadow-lg"
                  style={{ backgroundColor: secondaryColor }}
                >
                  <Phone className="w-3.5 h-3.5" />
                  <span>{lang === 'de' ? 'Termin Vereinbaren' : 'Prendre Rendez-vous'}</span>
                </a>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 relative rounded-3xl overflow-hidden border border-white/10 ring-1 ring-white/5 min-h-[420px] shadow-2xl">
              ...
              <div className="absolute bottom-6 left-6 right-6 p-5 rounded-2xl backdrop-blur-xl bg-black/80 border border-white/10 space-y-1">
                <span className="text-xs font-serif font-bold text-white block">{name}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="pt-2">
                <a 
                  href={`tel:${displayPhone}`}
                  className="px-6 py-3 text-black font-bold text-xs uppercase tracking-wider rounded-xl shadow-xl inline-flex items-center gap-2"
                  style={{ backgroundColor: secondaryColor }}
                >
                  <Phone className="w-4 h-4" />
                  <span>{lang === 'de' ? 'Jetzt Anrufen' : 'Appeler'}</span>
                </a>
              </div>
            </div>
      ...
      <section className="py-16 max-w-7xl mx-auto px-6 space-y-8">
        ...
        {servicesVariant === 'price-list-table' ? (
          /* Swiss Premium Menu / Price List Table */
      ...
      <section className="py-12 max-w-7xl mx-auto px-6">
        <div className="p-8 rounded-3xl border border-white/10 backdrop-blur-2xl space-y-6 shadow-2xl" style={{ backgroundColor: surfaceBg }}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <div className="text-xs font-mono uppercase text-amber-400 flex items-center gap-1.5">
                <MapPin className="w-4 h-4" />
                <span>{lang === 'de' ? 'Standort' : 'Adresse'}</span>
              </div>
              <div className="text-base font-serif font-bold text-white">{name}</div>
              <div className="text-xs text-zinc-300">{address}</div>
            </div>

            <div className="space-y-2">
              <div className="text-xs font-mono uppercase text-amber-400 flex items-center gap-1.5">
                <Phone className="w-4 h-4" />
                <span>{lang === 'de' ? 'Telefon' : 'Téléphone'}</span>
              </div>
              <div className="text-base font-mono font-bold text-white">{displayPhone}</div>
              <div className="text-xs text-zinc-300">{displayEmail}</div>
            </div>

            <div className="space-y-2">
              <div className="text-xs font-mono uppercase text-amber-400 flex items-center gap-1.5">
                <Clock className="w-4 h-4" />
                <span>{lang === 'de' ? 'Öffnungszeiten' : 'Horaires'}</span>
              </div>
              <div className="text-xs text-zinc-200">Montag - Freitag: 07:30 - 18:30</div>
              <div className="text-xs text-zinc-300">Samstag: 08:00 - 16:00</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-12 border-t border-white/10 text-center text-xs text-zinc-400 font-mono space-y-3 bg-black/60 backdrop-blur-2xl">
        <p>{name} · {address} · Tel: {displayPhone} · Email: {displayEmail}</p>
        <p className="text-[11px] text-zinc-500">© {new Date().getFullYear()} {name}. All rights reserved.</p>
      </footer>
    </div>
  );
}
